...
export function GridParkingItem({
  title,
  occupation,
  navigator,
  id,
}: Readonly<GridParkingItemProps>) {
  return (
    <button
      className="flex flex-col w-full h-full p-5 bg-sky-100 items-center justify-center gap-4"
      onClick={() => navigator({ to: `/parking/${id}` })}
    >
      <span className="text-sky-700 text-3xl font-semibold text-center">
        {title}
      </span>
